          ...
          <div class="     w-full overflow-x-auto   md:rounded-lg">
            <table ref="tableRef "
              ...
              <tr class="text-sm text-center ">
                ...
                <th scope="col"
                    class="px-2 py-3   cursor-pointer duration-300 hover:text-gray-500 hover:scale-[99%]"
                    @click="params.order_by='agency_level';params.dir=params.dir=='ASC'? 'DESC':'ASC'; params.page=1;getData()">
                  <div class="flex items-center justify-center">
                    <span class="px-2">    {{ __('agency_type') }} </span>
                    <ArrowsUpDownIcon class="w-4 h-4 "/>
                  </div>
                </th>
                <th scope="col"
                    class="px-2 py-3   cursor-pointer duration-300 hover:text-gray-500 hover:scale-[99%]"
                    @click="params.order_by='accesses';params.dir=params.dir=='ASC'? 'DESC':'ASC'; params.page=1;getData()">
                  <div class="flex items-center justify-center">
                    <span class="px-2">    {{ __('accesses') }} </span>
                    <ArrowsUpDownIcon class="w-4 h-4 "/>
                  </div>
                </th>
              ...
              <tr v-for="(d,idx) in data"
                ...
                <td
                    class="   items-center px-2 py-4 text-gray-900  ">
                ...
                <td class="px-2 py-4    " data-te-dropdown-ref>
                  <button
                      ...
                      :class="`bg-${getStatus('access', d.status).color}-100 hover:bg-${getStatus('access', d.status).color}-200 text-${getStatus('access', d.status).color}-500`">
                    {{ (d.accesses || []).length }}
                  </button>
                  <ul ref="statusMenu" data-te-dropdown-menu-ref
                    ...
                    <li v-for="(access,idx) in d.accesses" role="menuitem"

                        class="   cursor-pointer   text-sm   transition-colors hover:bg-gray-100">
                      <div class="flex items-center text-gray-700  px-6 py-2 justify-center ">
                        {{ access?.split(':').map(e => __(e)).join(':') }}
                      </div>
                      <hr class="border-gray-200 ">
                    </li>
