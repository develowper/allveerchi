<template>
  ...
  <div class="   ">
    ...
    <div
      ...
      <Squares2X2Icon class="h-4 w-4 mx-1 "/>
    ...
    <div
        ...
        id="treeSelectorModal"
        tabindex="-1"
        aria-labelledby="treeSelectorModalLabel"
        aria-hidden="true">
      <div
              ...
              <div
                ...
                <div class="flex flex-col mx-2   col-span-2 w-full     px-2">
                  <ul>
                    ...
                    <li v-for="(d,idx) in  items "
                        @click="selectItem(d)" v-show="  d.parent_id==currentItem.id"
                      ...
                      <div class="flex items-center">
                        <div class="mb-[0.125rem] block min-h-[1.5rem] pl-[1.5rem]" dir="ltr">
                          ...
                          <!--                          <label @click.stop-->
                          <label v-if="d.children.length>0"
                          ...
                          <input v-if=" d.children?.length==0"
                      ...
                      <ChevronLeftIcon v-if="d.children?.length>0" class="w-4 h-4"/>
                    </li>
                  </ul>
                </div>
...
export default {
  data() {
    return {
      currentLevel: 1,
      currentItem: {id: 0, parent_id: 0, level: 1},
      selecteds: [],
      selectedName: null,
      loading: false,
      items: this.data?.map(i => ({...i, name: `${i.name}(${i.id})`, parent_id: i.parent_id == null ? 0 : i.parent_id}
  ...
  mounted() {
    ...
    const modalEl = document.getElementById('treeSelectorModal');
      ...
      for (let idx in this.preload)
        for (let ix in this.data)
          if (this.preload[idx] == this.data[ix].id)
            this.selecteds.push({id: this.data[ix].id, name: `${this.data[ix].name}(${this.data[ix].id})`});
      this.$emit('update:modelValue', this.myMap(this.selecteds, (e) => e.id));

    }
    // for (let idx in this.cities)
    //   this.cities[idx].children = this.cities.filter((e) => e.parent_id == this.cities[idx].id).length > 0;
    // this.preload();
  ...
  methods: {
    ...
    reset() {
      this.currentLevel = 1;
      this.currentItem = {id: 0, parent_id: 0};
    },
    ...
    back() {
      ...
      if (this.currentItem.parent_id > 0) {
        this.currentItem = this.items.filter((el) => el.id == this.currentItem.parent_id)[0];
        this.currentLevel = this.currentItem.level;

      }
      // this.currentLevel = parent.level;
      else this.currentItem = {id: 0, parent_id: 0, level: 0};


    },
    selectItem(d) {
      if (this.items.filter((e) => d.id == e.parent_id).length == 0) {
    ...
    getItems(level, parent) {
      // this.log(level + "," + parent)
      this.filteredCities = [];
      for (const idx in this.items) {
        if (this.items[idx].level == level && this.items[idx].parent_id == parent)
          this.filteredData.push(this.items[idx]);
      }
