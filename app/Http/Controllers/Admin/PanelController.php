...
//    dd($middleware);
    ...
    protected function index(Request $request)
    {
        ...
        $partnership = Message::select('type', DB::raw('COUNT(*) AS count'))->groupBy('type')->get();
        ...
        $params = [
            ...
            'users' => array_map(function ($el) use ($users) {
                return ['title' => __($el['name']), 'color' => $el['color'] ?? 'gray', 'count' => optional($users->where('status', $el['name'])->first())->count ?? 0];
